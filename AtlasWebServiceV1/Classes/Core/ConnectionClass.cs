...
namespace PointsSystemWebService.Classes.Core
{
    public class ConnectionClass
    {
        ...
        {
            //Check If system Up and valid for use
            if (!Config.IsSystemUp)
            {
                //CheckLicense
                //ServiceMethod.CheckLicense();

                throw new Exception(Errors.LicenseExceeded.ToString());
            }
        ...
        public static SqlDataReader ExecuteStoredProcedure(string ProcedureName, bool isAPITestingMode, List<SqlParameter> Params,out SqlConnection connection)
        {
            ...
            cmd.Parameters.Add(new SqlParameter("LoggedUser", (isAPITestingMode) ? "7" : HttpContext.Current.Request.Headers["LoggedUser"]));
